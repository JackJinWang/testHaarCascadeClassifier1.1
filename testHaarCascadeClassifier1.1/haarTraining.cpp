...
MyCARTClassifier readXML(const char* xmlPath, MyCARTClassifier &strongClassifier)
{
	...
	while (surface)
	{
		...
		//	cout << surfaceChild->Name() << " = " << surfaceChild->GetText() << endl;
			...
			{
				/*
				XMLElement * haardetial = surfaceChild->FirstChildElement();
				while (haardetial)
				{
					if (strcmp(haardetial->Name(), NUMBER) == 0)
					{
						tempWeak.compidx = atoi(haardetial->GetText());
					}
					break;
				}
				*/
...
*读入XML
*/
MyCascadeClassifier readCascadeXML(const char* xmlPath, MyCascadeClassifier &classifier)
{
	int stage_number = 0;
	XMLDocument doc;
...
//	const char* cascade_name = "F:\\workplace\\test\\result\\1.7dt\\cascade.xml";
	...
	float negHit = 0;
	cascade = readCascadeXML(cascade_name, cascade);  //读取分类器
	...
	*/
	...
	predit_result = new int[sampleNumber]; //阶段预测
	pic = createMyMat(winsize.height+1,winsize.width+1,ONE_CHANNEL,INT_TYPE);
	for (int i = 0;i < sampleNumber;i++)
	{
		memcpy(pic->data.i, &training_data->sum.data.i[i * training_data->sum.width], sizeof(int)*(winsize.height + 1)*(winsize.width + 1));
		predit_result[i] = predictSignal(haar_features,pic,winsize,cascade);
	}
